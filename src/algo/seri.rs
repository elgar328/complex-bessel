...
mod tests {
    ...
    fn seri_i0_small_z() {
        ...
        assert!(
            (y[0].re - 1.0634833707413234).abs() < 1e-14,
            "I_0(0.5) = {}, expected 1.0634833707413234",
            y[0].re
        );
        assert_eq!(nz, 0);
    }
    ...
    fn seri_i1_small_z() {
        ...
        assert!(
            (y[0].re - 0.25789430539089628).abs() < 1e-14,
            "I_1(0.5) = {}",
            y[0].re
        );
        assert_eq!(nz, 0);
    ...
    fn seri_sequence_n3() {
        ...
        assert_eq!(nz, 0);
        assert!((y[0].re - 1.2660658777520084).abs() < 1e-14); // I_0(1)
        assert!((y[1].re - 0.5651591039924851).abs() < 1e-14); // I_1(1)
        assert!((y[2].re - 0.13574766976703831).abs() < 1e-14); // I_2(1)
    }

    #[test]
    fn seri_complex_arg() {
        // I_0(0.5+0.5i) computed via power series
        // Cross-verified with series: I_0(z) = sum (z/2)^{2k}/(k!)^2
        // z²/4 = 0.125i, first terms: 1 + 0.125i - 0.00390625 - ...
        ...
        assert_eq!(nz, 0);
        // Cross-check: zbesi route for same value
        let besi_result = crate::besi::zbesi(z, 0.0, crate::types::Scaling::Unscaled, 1).unwrap();
        assert!(
            (y[0].re - besi_result.values[0].re).abs() < 1e-14,
            "seri vs besi: re diff = {}",
            (y[0].re - besi_result.values[0].re).abs()
        );
        assert!(
            (y[0].im - besi_result.values[0].im).abs() < 1e-14,
            "seri vs besi: im diff = {}",
            (y[0].im - besi_result.values[0].im).abs()
        );
    }
}
