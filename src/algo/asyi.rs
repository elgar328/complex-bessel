...
mod tests {
    ...
    fn asyi_large_z() {
        ...
        assert_eq!(nz, 0);
        let expected = 4.355828255955561e7;
        let rel_err = (y[0].re - expected).abs() / expected;
        assert!(
            rel_err < 1e-12,
            "I_0(20) rel_err = {rel_err:.2e}, got {}",
            y[0].re
        );
    }
    ...
    fn asyi_scaled() {
        ...
        assert_eq!(nz, 0);
        let expected = 4.355828255955561e7 * (-20.0_f64).exp();
        let rel_err = (y[0].re - expected).abs() / expected.abs();
        assert!(rel_err < 1e-12, "scaled I_0(20) rel_err = {rel_err:.2e}");
    }
}
