...
mod tests {
    ...
    #[test]
    fn zunik_basic_i_function() {
        // z=2+0i, fnu=10, ikflg=1 (I function), ipmtr=0
        let zr = Complex64::new(2.0, 0.0);
        let result = zunik(zr, 10.0, 1, 0, TOL, None);

        // Verify basic properties:
        // zeta2 = fnu * sqrt(1 + (z/fnu)^2) = 10 * sqrt(1 + 0.04)
        let expected_sr = (1.04_f64).sqrt();
        let expected_zeta2 = 10.0 * expected_sr;
        assert!(
            (result.zeta2.re - expected_zeta2).abs() < 1e-12,
            "zeta2.re: got {}, expected {}",
            result.zeta2.re,
            expected_zeta2
        );
        assert!(result.zeta2.im.abs() < 1e-14);

        // phi should be real and positive for real z
        assert!(result.phi.re > 0.0);
        assert!(result.phi.im.abs() < 1e-14);

        // sum should be close to 1 for large fnu (leading term dominates)
        assert!(
            (result.sum.re - 1.0).abs() < 0.1,
            "sum.re: {}",
            result.sum.re
        );

        // cache should be valid
        assert!(result.cache.init >= 2);
    }

    #[test]
    fn zunik_basic_k_function() {
        // Same parameters but ikflg=2 (K function)
        let zr = Complex64::new(2.0, 0.0);
        let result = zunik(zr, 10.0, 2, 0, TOL, None);

        // zeta1, zeta2 should be the same as I function (they don't depend on ikflg)
        let result_i = zunik(zr, 10.0, 1, 0, TOL, None);
        assert!((result.zeta1.re - result_i.zeta1.re).abs() < 1e-14);
        assert!((result.zeta2.re - result_i.zeta2.re).abs() < 1e-14);

        // phi differs: K uses CON(2) = sqrt(pi/2)
        // I uses CON(1) = 1/sqrt(2*pi)
        // ratio should be CON(2)/CON(1) = pi
        let ratio = result.phi.re / result_i.phi.re;
        assert!(
            (ratio - core::f64::consts::PI).abs() < 1e-12,
            "phi ratio: {}",
            ratio
        );

        // K sum uses alternating signs
        assert!(result.sum.re != result_i.sum.re);
    }

    #[test]
    fn zunik_complex_argument() {
        // z=5+3i, fnu=20
        let zr = Complex64::new(5.0, 3.0);
        let result = zunik(zr, 20.0, 1, 0, TOL, None);

        // phi, zeta1, zeta2 should all be complex
        assert!(result.phi.im.abs() > 1e-10);
        assert!(result.zeta1.im.abs() > 1e-10);
        assert!(result.zeta2.im.abs() > 1e-10);
        assert!(result.cache.init >= 2);
    }
