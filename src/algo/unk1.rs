...
mod tests {
    ...
    #[test]
    fn zunk1_basic_k_only() {
        // MR=0: K function only (no analytic continuation)
        let z = Complex64::new(5.0, 1.0);
        let (y, nz) = zunk1(z, 90.0, Scaling::Unscaled, 0, 1, TOL, ELIM, ALIM);
        assert!(nz >= 0, "nz = {}", nz);
        assert!(y[0].re.is_finite());
        assert!(y[0].im.is_finite());
    }
    ...
    fn zunk1_k_sequence() {
        ...
        let (y, nz) = zunk1(z, 90.0, Scaling::Unscaled, 0, 3, TOL, ELIM, ALIM);
        assert!(nz >= 0);
        // At least one value should be nonzero
        let any_nonzero = y.iter().any(|v| v.re != 0.0 || v.im != 0.0);
        assert!(any_nonzero);
    }

    #[test]
    fn zunk1_analytic_continuation() {
        // MR=1: analytic continuation to left half plane
        let z = Complex64::new(-3.0, 1.0);
        let (y, nz) = zunk1(z, 90.0, Scaling::Unscaled, 1, 1, TOL, ELIM, ALIM);
        assert!(nz >= 0, "nz = {}", nz);
        assert!(y[0].re.is_finite());
    }
    ...
    fn zunk1_scaled() {
        ...
        let (y, nz) = zunk1(z, 90.0, Scaling::Exponential, 0, 1, TOL, ELIM, ALIM);
        assert!(nz >= 0);
        assert!(y[0].re.is_finite());
    }

    #[test]
    fn zunk1_real_argument() {
        // Real positive z, K should be real
        let z = Complex64::new(10.0, 0.0);
        let (y, nz) = zunk1(z, 90.0, Scaling::Unscaled, 0, 1, TOL, ELIM, ALIM);
        assert!(nz >= 0);
        if nz == 0 {
            assert!(
                y[0].im.abs() < 1e-10 * y[0].re.abs().max(1e-300),
                "imaginary part should be tiny: {}",
                y[0].im
            );
        }
    }
