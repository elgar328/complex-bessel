...
mod tests {
    ...
    #[test]
    fn acai_basic() {
        // Basic test: analytic continuation for fnu=1/3
        ...
        assert!(result.is_ok(), "zacai should succeed for basic input");
        let (y, nz) = result.unwrap();
        assert_eq!(y.len(), 1);
        assert!(nz >= 0);
    }
    ...
    fn acai_kode2() {
        ...
        assert!(result.is_ok(), "zacai should succeed for KODE=2");
    }
