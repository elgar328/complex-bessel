// Integration tests for Phase 2 (K Bessel function).
//
// Tests zbesk (upper interface) against Fortran TOMS 644 reference values
// stored in tests/reference_values/besk_f64.json.
//
// Tolerance: 1e-14 (same algorithm, bit-exact is not expected due to
// different operation ordering between Fortran and Rust).

use complex_bessel::besselk_seq;
use complex_bessel::types::Scaling;
use num_complex::Complex;
use serde::Deserialize;

// ── JSON schema ──

#[derive(Deserialize)]
#[allow(dead_code)]
struct RefFile {
    generator: String,
    machine: String,
    tests: Vec<RefTest>,
}

#[derive(Deserialize)]
struct RefTest {
    function: String,
    label: String,
    inputs: RefInputs,
    outputs: RefOutputs,
}

#[derive(Deserialize)]
struct RefInputs {
    z_re: f64,
    z_im: f64,
    fnu: f64,
    kode: i32,
    n: i32,
}

#[derive(Deserialize)]
#[allow(dead_code)]
struct RefOutputs {
    nz: Option<i32>,
    ierr: Option<i32>,
    // zbesk outputs
    cy_re: Option<Vec<f64>>,
    cy_im: Option<Vec<f64>>,
    // zbknu outputs
    y_re: Option<Vec<f64>>,
    y_im: Option<Vec<f64>>,
}

// ── Tolerance ──
// Same algorithm (Fortran vs Rust) — expect near bit-exact results.
// Some cases (e.g., K_0.25(1.5+0.5i)) show ~1.4e-14 due to different
// floating-point operation ordering. Use 2e-14 to accommodate.

const FORTRAN_TOL: f64 = 2e-14;

// ── Helper: complex relative error ──

fn assert_complex_rel(label: &str, got: Complex<f64>, re_ref: f64, im_ref: f64, tol: f64) {
    let expected = Complex::new(re_ref, im_ref);
    let scale = expected.norm();
    if scale == 0.0 {
        // Both should be zero
        assert!(
            got.re.abs() < tol && got.im.abs() < tol,
            "{label}: expected ~0, got ({}, {})",
            got.re,
            got.im
        );
        return;
    }
    let err = (got - expected).norm() / scale;
    assert!(
        err < tol,
        "{label}: relative error {err:.2e} exceeds tolerance {tol:.0e}\n  expected: ({re_ref:.17e}, {im_ref:.17e})\n  got:      ({:.17e}, {:.17e})",
        got.re,
        got.im,
    );
}

// ── Test: zbesk (upper interface) ──

#[test]
fn besk_vs_fortran() {
    let json = include_str!("reference_values/besk_f64.json");
    let refs: RefFile = serde_json::from_str(json).expect("failed to parse besk_f64.json");

    let zbesk_tests: Vec<&RefTest> = refs
        .tests
        .iter()
        .filter(|t| t.function == "zbesk")
        .collect();

    assert!(
        !zbesk_tests.is_empty(),
        "no zbesk tests found in reference file"
    );

    for t in &zbesk_tests {
        let z = Complex::new(t.inputs.z_re, t.inputs.z_im);
        let fnu = t.inputs.fnu;
        let scaling = match t.inputs.kode {
            1 => Scaling::Unscaled,
            2 => Scaling::Exponential,
            _ => panic!("{}: unknown kode {}", t.label, t.inputs.kode),
        };
        let n = t.inputs.n as usize;

        let result = besselk_seq(fnu, z, n, scaling)
            .unwrap_or_else(|e| panic!("{}: unexpected error {:?}", t.label, e));

        let cy_re = t.outputs.cy_re.as_ref().expect("missing cy_re");
        let cy_im = t.outputs.cy_im.as_ref().expect("missing cy_im");

        assert_eq!(
            result.values.len(),
            n,
            "{}: expected {} values, got {}",
            t.label,
            n,
            result.values.len()
        );
        assert_eq!(
            result.underflow_count,
            t.outputs.nz.unwrap_or(0) as usize,
            "{}: nz mismatch",
            t.label
        );

        for j in 0..n {
            let sub_label = if n > 1 {
                format!("{}[{}]", t.label, j)
            } else {
                t.label.clone()
            };
            assert_complex_rel(
                &sub_label,
                result.values[j],
                cy_re[j],
                cy_im[j],
                FORTRAN_TOL,
            );
        }
    }

    eprintln!(
        "  besk_vs_fortran: {} zbesk test cases passed (tol={:.0e})",
        zbesk_tests.len(),
        FORTRAN_TOL
    );
}

// ── Test: zbknu (core subroutine) via zbesk ──
// Since zbknu is pub(crate), we test it indirectly via zbesk.
// For single-value unscaled calls, zbesk delegates directly to zbknu,
// so the results should match exactly.

#[test]
fn bknu_vs_fortran_via_zbesk() {
    let json = include_str!("reference_values/besk_f64.json");
    let refs: RefFile = serde_json::from_str(json).expect("failed to parse besk_f64.json");

    let bknu_tests: Vec<&RefTest> = refs
        .tests
        .iter()
        .filter(|t| t.function == "zbknu")
        .collect();

    assert!(
        !bknu_tests.is_empty(),
        "no zbknu tests found in reference file"
    );

    for t in &bknu_tests {
        let z = Complex::new(t.inputs.z_re, t.inputs.z_im);
        let fnu = t.inputs.fnu;
        let scaling = match t.inputs.kode {
            1 => Scaling::Unscaled,
            2 => Scaling::Exponential,
            _ => panic!("{}: unknown kode {}", t.label, t.inputs.kode),
        };
        let n = t.inputs.n as usize;

        let result = besselk_seq(fnu, z, n, scaling)
            .unwrap_or_else(|e| panic!("{}: unexpected error {:?}", t.label, e));

        let y_re = t.outputs.y_re.as_ref().expect("missing y_re");
        let y_im = t.outputs.y_im.as_ref().expect("missing y_im");

        assert_eq!(
            result.values.len(),
            n,
            "{}: expected {} values, got {}",
            t.label,
            n,
            result.values.len()
        );

        for j in 0..n {
            let sub_label = if n > 1 {
                format!("{}[{}]", t.label, j)
            } else {
                t.label.clone()
            };
            assert_complex_rel(&sub_label, result.values[j], y_re[j], y_im[j], FORTRAN_TOL);
        }
    }

    eprintln!(
        "  bknu_vs_fortran_via_zbesk: {} zbknu test cases passed (tol={:.0e})",
        bknu_tests.len(),
        FORTRAN_TOL
    );
}
